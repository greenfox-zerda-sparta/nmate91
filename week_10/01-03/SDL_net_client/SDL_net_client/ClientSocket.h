...
class ClientSocket {
	private:
		...
		string serverHostname; // The host name of the server
	...
	public:
		...
		void connectToServer(); // Function to poll for clients connecting

		string checkForIncomingMessages(); // Function to check the server for incoming messages
		void displayMessage(string &receivedMessage); // Function to display a received message

		// Function to send a message to the server
		//void sendOutgoingMessage();

		void displayPrompt(); // Function display the prompt + any input that hasn't been sent yet
		void getUserInput(); // Function to get keypresses in a non-blocking manner
	
		string getCurrentUserInputContents(); // Function to get the current contents of our outgoing message

		bool getShutdownStatus(); // Function to return the shutdown status, used to control when to terminate
};
